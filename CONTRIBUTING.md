...
The development server runs at [http://127.0.0.1:5175](http://127.0.0.1:5175) and hot-reloads source changes. It also accepts the machine's Tailscale MagicDNS name under `*.ts.net`. The browser suite builds its own fresh production bundle before starting the preview server.
